Public Class DaoSingleSignOn
    Inherits AbstractTableDao

#Region "定数"
    Public Const TABLE_NAME As String = "T_JP_SingleSignOn"
#End Region
...
#Region "tokenによる取得"
    Public Function GetCountByToken(ByVal pToken As String) As Integer
        Const SQL_QUERY As String = "SELECT COUNT(*) " & _
                                      "FROM " & TABLE_NAME & " " & _
                                     "WHERE token = @token"
        Try
            Me.DbManager.SetSqlCommand(SQL_QUERY, CommandType.Text)
            Me.DbManager.SetCmdParameter("@token", SqlDbType.VarChar, ParameterDirection.Input, pToken)
            Return Utilities.N0Int(Me.DbManager.ExecuteScalar())
        Catch ex As ElsException
            ex.AddStackFrame(New StackFrame(True))
            Throw ex
...
#Region "tokenによる取得"
    Public Function GetByToken(ByRef pDt As DataTable, _
                            ByVal pToken As String) As Integer
        Const SQL_QUERY As String = "SELECT * " & _
                                      "FROM " & TABLE_NAME & " " & _
                                     "WHERE token = @token"
        Try
...
#Region "更新"
    Public Function Update(ByRef pDr As DS_USER.T_JP_SingleSignOnRow) As Integer
        Const SQL_QUERY As String = "UPDATE " & TABLE_NAME & " " & _
                                       "SET " & _
                                             "user_id = @user_id " & _
                                            ",token = @token " & _
                                            ",RedirectPageID = @RedirectPageID " & _
